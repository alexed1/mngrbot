...
Pmbot.prototype._onMessage = function (message){
  if (this._isChatMessage(message) &&
      ...
      this._showGameMenu(message.channel);
    }
};
...
Pmbot.prototype.saveOAuthToken = function (token, team) {
  this.db.run('INSERT INTO tokens(token, team) VALUES(?, ?)', token, team, function(err, record) {
    if (err) {
      throw new Exception('DATABASE ERROR:' + JSON.stringify(err));
    }
  });
};

Pmbot.prototype.getTokenByTeam = function (team) {
  this.db.get('SELECT token FROM tokens WHERE team = ?', team, function(err, record) {
    if (err) {
      throw new Exception('DATABASE ERROR:' + JSON.stringify(err));
    }
    if (record)
...
Pmbot.prototype._welcomeMessage = function() {
  ...
  this.postMessageToChannel(this.settings.initChannel, "Hi, I'm PMbot, capable of providing automated product manager services 24/7. " +
...
Pmbot.prototype._showGameMenu = function(channelId) {
  ...
  this.postMessageToChannel(channel, "", {as_user: true, attachments: [buttons]});
}
