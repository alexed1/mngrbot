...
var Pmbot = function Constructor(settings) {
  ...
  this.settings.name = this.settings.name || 'mngrbot2';
...
Pmbot.prototype._onMessage = function (message) {
  ...
  //   ) {
  //
  //     this.__respondWithText(message);
  //     setTimeout(() => this._showGameMenu(), 500);
  //   }
  //     //this._welcomeMessage();
  // };

Pmbot.prototype._processMessage = function (message) {
  ...
  if (this._isButtonClick(message)) {
    this._generateResponseFromText(message.message.text, message.channel);
    setTimeout(() => this._showGameMenu(), 500);
  }
}
...
Pmbot.prototype._loadBotUser = function () {
  ...
  Pmbot.prototype._welcomeMessage = function () {
    this.postMessageToChannel(this.settings.initChannel, "Hi, I'm Mngrbot, capable of providing automated eng manager services 24/7. " +
      "\n I can do the following: " +
      "\n provide a morale `boost` ," +
      "\n `chitchat` about tech (mostly javascript)," +
      "\n or 'grumble' entertainingly about how much easier engineers have it today` ", {
        as_user: true
      });
  };
  ...
  Pmbot.prototype._showGameMenu = function (channelId) {
    var channel = (channelId != null) ? self._getChannelById(channelId).name : this.settings.initChannel;
